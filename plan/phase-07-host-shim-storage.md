...
## Цель
Добавить в Host Shim абстракцию для работы с файловой системой хост-ОС: чтение, запись, наблюдение (watcher), блокировки, и доступ к съёмным носителям (USB). После этого этапа CORE OS умеет читать и писать файлы на хосте, отслеживать изменения в папках, и импортировать данные с USB.
...
- Все пути — абсолютные или относительные к `CORE_ROOT` (рабочая директория CORE OS на хосте).
...
- **Mirror Folders:** watcher используется для Mirror Folders — папок на хост-ФС, которые отображаются в VFS CORE OS [См. layer-5 §3.3, layer-8 §5.4].
...
- По умолчанию приложение видит только свою песочницу (`CORE_ROOT/apps/<app-id>/`).
