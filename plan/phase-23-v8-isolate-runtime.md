...
## Цель
Создать runtime для нативных приложений Workspace на базе V8 Isolates (через Bun). После этого этапа приложения level 3–5 могут запускаться в sandbox, иметь доступ к `@workspace/*` API, и взаимодействовать с системой через capabilities.
...
- **Ключевые зависимости:** встроенный V8 в Bun, кастомный `@workspace/*` SDK
- **Целевые ОС:** Windows, macOS, Linux, Android

## Зависимости
- **Этап 12** — Micro-Kernel: WORKSPACE (IPC, event loop, SQLite).
- **Этап 13** — Micro-Kernel: Security (capability checks, sandbox policies).
- **Этап 14** — Micro-Kernel: VFS (файловый доступ).
- **Этап 22** — App Registry (установленные приложения, workspace.json, capabilities).
...
- **Загрузка кода:** entry point (`workspace.json:entry`) транспилируется и выполняется в isolate.
- **Контекст:** каждый isolate получает `globalThis.WORKSPACE` — API-объект с методами, разрешёнными capabilities.
...
### 20.2 Sandbox
- **Memory limit:** каждый isolate имеет лимит RAM (по умолчанию 128 МБ, настраивается в `workspace.json`).
...
- **Filesystem sandbox:** `WORKSPACE.fs.read(path)` — path ограничен `WORKSPACE_ROOT/apps/<app-id>/` + explicitly granted папками.
...
### 20.3 @workspace/* API
- **@workspace/fs:** `read`, `write`, `list`, `watch` — доступ к VFS с capability-check.
- **@workspace/net:** `fetch` (ограниченный), `websocket` — сеть.
- **@workspace/ui:** `render(tree)` — отправка React-подобного дерева компонентов в Display Server. Display Server рендерит его как texture для окна приложения.
- **@workspace/graphics:** `createCanvas()`, `draw()` — low-level WebGPU API для level 5 приложений (canvas texture отправляется в Display Server).
- **@workspace/audio:** `play()`, `capture()` — аудио через Host Shim.
- **@workspace/notifications:** `send(title, body)` — системные уведомления.
- **@workspace/intent:** `emit(intent)` — отправка Intent в систему (например, "открой файл").
- **@workspace/contacts:** `list()`, `search()` — адресная книга (с `contacts:read`).
- **@workspace/messenger:** `send(peer, message)` — отправка сообщения.
- **@workspace/mock:** npm-пакет для тестирования приложений вне Workspace (dev dependency).
...
- **Диалог:** модальное окно (рендерится Display Server как Overlay Layer):
  ...
  - Список запрашиваемых capabilities с описанием (из `workspace.json:permissions_ui`).
...
- **@workspace/ui:** React-подобный API. Компоненты: `View`, `Text`, `Image`, `Button`, `TextInput`, `ScrollView`.
  - Каждый компонент — описание (props + children), не DOM-узел.
  - Display Server превращает `RenderNode` tree в GPU draw calls (этап 8–9).
- **@workspace/graphics (level 5):** Raw WebGPU API. Приложение создаёт `RenderTexture`, рисует в неё через WebGPU commands, и отправляет texture handle в Display Server для compositing.
...
|---------|----------|------|
| Launch app | Запуск изолейта | Открыть Workspace.notes → isolate создан, код выполнен |
| @workspace/fs.read | Чтение файла | Вызов → содержимое файла (sandboxed) |
| @workspace/ui.render | Отрисовка UI | Отправка tree → Display Server рендерит окно |
...
- **Вход:** этап 19 (App Registry) — workspace.json, capabilities, installed apps.
...
- [ ] `@workspace/fs.read` возвращает файл из песочницы.
- [ ] `@workspace/ui.render` отображает окно через Display Server.
...
- [layer-11-developer-reference.md](../layers/layer-11-developer-reference.md) — @workspace/* API, App Model
