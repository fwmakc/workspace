...
6. **Никаких заглушек — только чистая реализация.** Никаких `TODO`, `unimplemented!()`, пустых тел функций, фейковых структур. Если этап нельзя реализовать полностью — реализуется что возможно, а остаток фиксируется в файле этапа. Существующие заглушки из предыдущих этапов заменяются при переходе к соответствующему этапу. **Исключение:** macOS и iOS — если требуется физическое устройство Apple для проверки, реализация может компилироваться и проходить CI без ручной проверки на железе. Код при этом обязан быть полным и корректным.
