...
## Цель
Создать ядро системы на TypeScript/Bun: runtime, IPC-мост с Host Shim, SQLite-схему данных, и базовый event loop. После этого этапа CORE OS имеет работающий бэкенд, который получает события от Host Shim, хранит данные в SQLite, и может запускать простейшие скрипты.
...
Micro-Kernel — это сердце CORE OS. Он управляет всей логикой: проекты, приложения, данные, настройки, безопасность. Он работает внутри Bun runtime и общается с Host Shim (Rust) через zero-copy IPC.
...
### 10.2 SQLite Schema
- База данных `core.db` в `CORE_ROOT/`.
...
- `CORE_ROOT` определяется автоматически:
  - Windows: `%LOCALAPPDATA%\CoreOS\`
  - macOS: `~/Library/Application Support/CoreOS/`
  - Linux: `~/.local/share/CoreOS/`
...
- [layer-4-installation-scenarios.md](../layers/layer-4-installation-scenarios.md) — First run, CORE_ROOT
