...
## Цель
Создать полноценный композитор окон (scene graph, damage tracking, effects) и реализовать Game Mode — режим прямого доступа к GPU для приложений. После этого этапа CORE OS умеет отображать несколько окон с наложением, анимациями, blur, и переключаться в эксклюзивный игровой режим.
