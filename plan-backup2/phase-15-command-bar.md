...
## Цель
Создать Command Bar — центральную точку ввода CORE OS. После этого этапа пользователь может набирать текст, видеть подсказки, переключаться между режимами, и взаимодействовать с системой через единую строку ввода.
