# Layer 2 — ИИ-слой и Intent API (Workspace.Mind)
...
ИИ в WORKSPACE — не чат-бот, не помощник, не «Сири». Это автоматика, которая связывает приложения и систему через типизированные команды. Пользователь говорит «сделай» — система делает, не задавая лишних вопросов.
...
| Компонент | Требование |
|-----------|------------|
| Процессор | Apple Silicon M1+ / Intel WORKSPACE i7+ / AMD Ryzen 7+ |
| RAM | 16 GB |
...
### System Settings → AI (Workspace.Mind)

**Главный переключатель:**
- `Workspace.Mind: Включён / Выключен`
- При выключении Workspace.Mind: Voice Engine не загружается, облачные модели и генеративный UI отключены. **Intent API (базовый) остаётся активным** — Command Bar понимает текстовые команды через локальный rule-based парсер.
...
- Wake-word: «WORKSPACE» / кастомное (3-4 слога)
...
- **Wake-word:** «WORKSPACE» активирует прослушивание. В Exclusive Mode — push-to-talk или постоянное прослушивание с шумоподавлением.
...
1. Wake-word («WORKSPACE») или push-to-talk.
...
```typescript
import { mind, voice, ai } from '@workspace/ai';
...
### Конфигурация моделей (workspace.json)
...
```bash
# Установка модели
workspace-cli ai model pull llama3.1:8b
workspace-cli ai model pull phi4:14b
workspace-cli ai model pull qwen2.5:7b
...
workspace-cli ai backend set vllm --model /path/to/model --gpu-memory-utilization 0.8
```
...
```bash
workspace-cli ai model register \
  --name my-company-llm \
...
- Доступ только через `@workspace/*` wrappers и Intent API.
...
| `ai:configure` | Owner | Изменение настроек Workspace.Mind |
...
Owner просматривает историю через Command Bar → «История AI» или Workspace.Backoffice → Audit → Category: AI.
...
| Команда | Действие |
|---------|----------|
| «Сделай музыку тише» | Сигнал в Workspace.Audio, плеер не открывается |
...
3. Отрисовывает временный виджет в пространстве через `@workspace/graphics`.
...
| [Layer: Устройства](layer-5-devices.md) | Whisper на NPU, локальный инференс на GPU, WORKSPACE-Plug как hardware target |
