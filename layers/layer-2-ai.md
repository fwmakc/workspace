...
- При выключении: Voice Engine не загружается, Command Bar работает только с клавиатуры, Intent API неактивен.
...
Важное разделение:
- **Ядро (Kernel, VFS):** Только чистая математика (CRDT). Детерминировано. Если не может свести данные → создаёт именованную ветку (как git branch).
...
| [Layer: Business](layer-9-business-model.md) | AI как киллер-фича для входа, B2B SaaS, обучение и сертификация |
