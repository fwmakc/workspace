...
На каком железе работает CORE OS. Минимальные, рекомендуемые и максимальные конфигурации. Оптимизации под слабое железо и специфические устройства.
...
Для устройств, которые физически не могут запустить полный CORE, но работают как "умный терминал".
...
- Remote Isolate Call — вся логика выполняется на удалённом узле (Core Base, ПК, сервер)
...
### 2. Рекомендуемая (Полноценный CORE)
...
### 3. Максимальная (Core Base / Разработка / Enterprise)
...
| Сеть | Ethernet 1 Gbps, статический IP (для Core Base) |

**Что работает:**
- Core Base (всегда включённый узел, seeding для других устройств)
...
CORE OS использует **WebGPU** (через wgpu-native) для рендеринга. Это критичный компонент.
...
| Модель | Статус | Режим |
|--------|--------|-------|
| Pi 4 (4GB) | ✅ Полноценный CORE | Рекомендуемая конфигурация |
| Pi 5 (8GB) | ✅ Отлично | Максимальная конфигурация |
| Pi Zero 2W | ⚠️ Lazy Boot | Headless Logic + Remote Isolate Call |
| Pi 3 | ⚠️ Ограниченно | Возможен полный CORE, но медленно (V8 Isolates тяжелы) |

**Особенности:**
- Зашифрованный rootfs (LUKS) по умолчанию для Core Base
...
| A15+ (iPad) | ✅ Хорошо. Полноценный CORE, ограничения iOS на фоновые процессы |
| A12 и старше | ⚠️ Ограниченно. Возможен Lazy Boot или ограниченный полный CORE |

### Windows-планшеты / 2-in-1

| Характеристика | Статус |
|----------------|--------|
| Intel Atom / Celeron | ⚠️ Lazy Boot или медленный полный CORE |
| Intel Core i3+ | ✅ Полноценный CORE |
...
CORE OS разделяет ИИ на **обязательный** (базовый Intent Parser) и **опциональный** (Core.Mind).
...
### Опциональный: Core.Mind (полный AI-стек)
...
**Ограничения при отключении Core.Mind:**
...
При первом запуске CORE OS проводит автоматический тест:
...
**Результат:**
- Все тесты пройдены → "Полноценный CORE"
...
|----------|---------------------|
| Домашний пользователь (1–3 устройства) | Любой ПК/ноутбук 2020+, Raspberry Pi 4 как Core Base |
...
| Образование (дешёвые планшеты) | Lazy Boot + мощный Core Base в классе |
| Разработчик (core-dev, TSCLANG) | Мощный ПК (8+ ядер, 32GB RAM, дискретная GPU) |
